...
green_times = {"empty road": 1, "normal traffic": 3, "heavy traffic": 5, "emergency vehicle ": 10}
current_object = None
...
def classify_image(image):
    ...
    if index < len(class_names):
        ...
        # Check if the confidence score is greater than 0.90
...
def start_timer(seconds):
    ...
    if seconds > 0:
        root.after(seconds * 1000, countdown, seconds)
...
def countdown(seconds):
    if seconds > 0:
        ...
        update_traffic_light("")  # Reset traffic light color
...
def capture_from_webcam():
    cap = cv2.VideoCapture(0) # change for camera 1 for outdide 0 for inbuild
    if not cap.isOpened():
...
title_label = tk.Label(title_frame, text="Traffic Light Control System", font=("Arial", 16, "bold"), bg="black")
title_label.pack()
...
input_label = tk.Label(input_frame, text="Input Image", font=("Arial", 14, "bold"), bg="black")
input_label.pack()
...
result_label = tk.Label(result_frame, text="", font=("Arial", 16), bg="black")
result_label.pack()

# Create a label for Confidence Score
confidence_label = tk.Label(root, text="", font=("Arial", 11), bg='black')

# Create a button to select an image
select_button = tk.Button(root, text="Select Image", command=select_image, bg="#4CAF50", foreground='black', font=("Arial", 12))
select_button.pack(pady=10)

# Create a button to capture image from webcam
webcam_button = tk.Button(root, text="Start Live Stream", command=capture_from_webcam, bg="#4CAF50", foreground='black', font=("Arial", 12))
webcam_button.pack(pady=10)

# Create a traffic light
traffic_light_canvas = tk.Canvas(root, width=80, height=200, bg="#f0f0f0", highlightthickness=0)
